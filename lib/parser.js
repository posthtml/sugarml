...
module.exports = (tokens) => {
  ...
  function walk () {
    ...
    // if there's an indent at the same level as
    ...
    if (token && token.type === 'tag') {
      ...
      // once we are done with the whole subtree, return to root
      return node
    }
    ...
    if (token && token.type === 'newline') {
      ...
      // we reset the indent level before moving forward
      if (token && token.type === 'tag') {
        indentLevel = 0
      }
      ...
      if (token && token.type === 'indent') {
        ...
        if (token.value > indentLevel) {
          indentLevel = token.value
          next()
          return walk()
        }
  ...
  // run the parser
  const root = []
  while (current < tokens.length) { root.push(walk()) }
  return root
