module.exports = function Lexer (input) {
  ...
  function classId () {
    if (char === '#' || char === '.') {
      ...
      if (!lastToken || !lastToken.name === 'tag') {
        addToken('tag', 'div')
      }
  ...
  function indentOrTagContent () {
    if (char.match(/\s/) && !char.match(/\n/)) {
      ...
      if (['tag', 'attributeKey', 'attributeValue'].indexOf(lastToken.name) > -1) {
        // move past the space
        next()
  ...
  function nestedInlineTag () {
    if (char && char === ':') {
      // move past the colon
      next()
      // is there's a space for padding, move past this
      if (char.match(/\s/)) next()
   ...
   * @param {String} name - name/type of the token, ex. "tag"
   * @param {String} value - token's contents/value, ex. "div"
   */
  function addToken (name, value) {
    tokens.push({name, value, line, col})
  }
